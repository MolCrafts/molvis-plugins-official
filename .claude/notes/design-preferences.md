...
`npm test -w <workspace>` on that package — **not** full-suite, **not**
cross-module regression. The full suite (`npm test`) is a CI net; it is not
how you green a unit during design.
...
- **Reimplementing host chrome** (dialog scrim, focus trap, panels) when
  `api.dialogs` / `api.panels` / commands exist.
- **Installing meta + the same child package** in one host session (duplicate
  toolbar / panels).
...
Unit tests under each package's `tests/`, path mirroring source, types mirroring
types. Single-function tests. One module → its mirrored tests only; unit green
does not require the full suite. Details: `tester` agent.
